...
from dataset_noaugmentation import SceneTextDataset
...
def do_training(data_dir, model_dir, device, ufo_name, image_size, input_size, num_workers, batch_size,
    ...
    # 요거 수정하면 폴더명, wandb name 바뀜
    wandb_name = f'noaug_{args.ufo_name}_epoch={args.max_epoch}_512_resize'
    wandb.init(config=wandb_config(args),project='Data-Centric', entity='cv11_aivengers',name=wandb_name) #수정
    print(wandb_name)
    model.train()
    val_loss = float("inf")
    # model_dir 수정
    # wandb_name = f'noaug_{args.ufo_name}_epoch={args.max_epoch}'
