...
def do_training(data_dir, model_dir, device, ufo_name, image_size, input_size, num_workers, batch_size,
    ...
    wandb.init(config=wandb_config(args),project='Data-Centric', entity='cv11_aivengers',name=f'{args.ufo_name}_epoch={args.max_epoch}_brightness') #수정
    
    model.train()
    val_loss = float("inf")
    # model_dir 수정
    wandb_name = f'{args.ufo_name}_epoch={args.max_epoch}_brightness'
